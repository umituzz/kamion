...
const UserDashboard = () => {
	...
	useEffect(() => {
		(async () => {
			setLoading(true);
			try {
				const response = await axios.get(`http://localhost/api/checkSetting?user=${userId}`);
	...
	const handleCheckbox = async (event) => {
		...
		if (event.target.checked) await axios.post("http://localhost/api/storeSetting", { user_id: userId, name: checkedVal, type: nameVal });
		else await axios.post("http://localhost/api/deleteSetting", { user_id: userId, name: checkedVal, type: nameVal });
	};
